...
const TrendingDesignCard = ({ design }: { design: TrendingDesign }) => (
  <div className="overflow-hidden rounded-lg shadow-md hover:shadow-lg transition-shadow">
    ...
    <div className="p-4">
      <h3 className="font-medium mb-2">{design.title}</h3>
      <p className="text-sm text-gray-600 mb-2">{design.location}</p>
      <p className="font-bold">₹{design.price} Lacks</p>
    </div>
  </div>
);

const DesignMainPageComponent = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <nav className="flex mb-4 text-sm">
        ...
        <span className="text-gray-900">Designs</span>
      </nav>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {trendingDesigns.map((design) => (
            <TrendingDesignCard key={design.id} design={design} />
          ))}
        </div>
      </section>
    </div>
...
export default DesignMainPageComponent;
